...
static int ISP_ProcShow(osal_proc_entry_t *s)
{
    ...
    osal_seq_printf(s, " %15s" " %15s" " %15s" " %15s"  " %15s\n",  "proc_param",  "update_pos", "pwm_num", "lsc_update_mode");
	osal_seq_printf(s, " %15u" " %15u" " %15u" " %15u"  " %15u\n",   proc_param,    update_pos,   pwm_num,   lsc_update_mode);
...
HI_VOID ISPCheckModParam(HI_VOID)
{
	if (0 > pwm_num || 3 < pwm_num)
	  {
		  osal_printk("-----ModuleParam: pwm_num %d is illegal,should be [0,3], it will be set to 1!\n",pwm_num);
		  pwm_num = 1;
	  }
	if (0 > proc_param )
	  {
		  osal_printk("-----ModuleParam: proc_param %d is illegal,should be [0, +∞], it will be set to 0!\n",proc_param);
		  proc_param = 0;
	  }
    ...
    if (0 > lsc_update_mode  || 1 < lsc_update_mode)
	  {
		  osal_printk("-----ModuleParam: lsc_update_mode %d is illegal,should be [0, 1], it will be set to 0!\n",lsc_update_mode);
		  lsc_update_mode = 0;
	  }
}
