...
#endif
static struct i2c_client* tlv_client = NULL;
...
struct i2c_client* hi_tlv320aic31_i2c_client_init(void)
{
    ...
    if (NULL == client){
        printf("malloc i2c_client err. \n");
        return NULL;
    }

    memset(client, 0 ,sizeof(struct i2c_client));
    snprintf(client->name, 32, "dev/i2c_client-tlv");
    client->addr = 0x30;

    ret = client_attach(client, TLV320AIC31_I2C_ADAPTER_ID);
    if (ret){
        printf("client_attach err. \n");
        free(client);
        return NULL;
    }
...
static
#endif
void __exit tlv320aic31_exit(void)
{
    unsigned int i;
