...
static HI_S32 multiCipher_DES_ECB_ENC()
{
	HI_S32 s32Ret = HI_SUCCESS;
...
static HI_S32 multiCipher_DES_ECB_DEC()
{
	HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 8;
    HI_U32 u32Testcached = 0;
...
static HI_S32 multiCipher_ex_AES_CBC_ENC()
{
	HI_S32 s32Ret = HI_SUCCESS;
    ...
    memcpy(CipherCtrl.u32IV, aes_IV, 16);
    memcpy(CipherCtrl.u32Key, aes_key, 16);      
    
    s32Ret = HI_UNF_CIPHER_EncryptMultiEx(hTestchnid, &CipherCtrl, cipherDataArray, 3);
    if(HI_SUCCESS != s32Ret)
    {
    	HI_ERR_CIPHER("Cipher encryptMulti failed.\n");
    	goto __CIPHER_EXIT__;
...
static HI_S32 multiCipher_ex_AES_CBC_DEC()
{
	HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 16;
    HI_U32 u32Testcached = 0;
    ...
    s32Ret = HI_UNF_CIPHER_DecryptMultiEx(hTestchnid, &CipherCtrl, cipherDataArray, 3);
    if(HI_SUCCESS != s32Ret)
    {
    	HI_ERR_CIPHER("Cipher DecryptMulti failed.\n");
    	goto __CIPHER_EXIT__;
...
int sample_mutiltcipher()
{
    printf("mutli-cipher not support!!!\n");
    return HI_SUCCESS;
}
#endif
