...
{
    HI_U32* pu32SoftChanId;
    HI_S32 s32Ret = 0;
    pu32SoftChanId = (HI_U32*)pParam;
    s32Ret = g_stCipherOsrChn[*pu32SoftChanId].g_bDataDone != HI_FALSE;
    return s32Ret;
        ...
        if (stDestMmzBuf.pu8StartVirAddr != 0)
        {
            ...
            else
            {
                u32Cnt = osal_wait_event_timeout(&g_stCipherOsrChn[u32SoftChanId].cipher_wait_queue,
                            DRV_CIPHER_WaitConditionCallBack, &u32SoftChanId, CIPHER_TIME_OUT);
            }
            if (0 == u32Cnt)
            {
...
HI_S32 DRV_CIPHER_EncryptMulti(CIPHER_DATA_S *pstCIData)
{
    ...
    if (0== osal_wait_event_timeout(g_stCipherOsrChn[softChnId].cipher_wait_queue,
                DRV_CIPHER_WaitConditionCallBack, &softChnId, CIPHER_TIME_OUT))
    {
