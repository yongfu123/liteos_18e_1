...
#define OSDRV_MODULE_VERSION_STRING "HISI_WDT-MDC030001 @Hi3518v200"
...
void *reg_ctl_base_va = NULL;
void *reg_wdt_base_va = NULL;
...
static int  hidog_init(void)
{
    ...
    if(!nodeamon) {
        ...
        if(ret <0) 
        {
            osal_printk("create hidog_deamon failed! ret = 0x%x\n", ret);
            return -1;
        }
#else
...
int watchdog_init(void)
{
    ...
    if ((NULL == reg_wdt_base_va) || (NULL == reg_ctl_base_va))
    {
        reg_wdt_base_va = OSAL_IO_ADDRESS(HIWDT_BASE);
        reg_ctl_base_va = OSAL_IO_ADDRESS(HISILICON_SCTL_BASE);
    }
