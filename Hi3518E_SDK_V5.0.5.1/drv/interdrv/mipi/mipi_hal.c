...
void* gpMiscRegBase = NULL;
void* gpCrgRegBase = NULL;

/****************************************************************************
 * Define the macro of the module mipi                                      *
 ****************************************************************************/
#define MIPI_BASE_ADDR 0x20680000
#define MISC_BASE_ADDR 0x20120000
#define CRG_BASE_ADDR 0x20030000
...
unsigned int mipi_drv_init_reg(void)
{
    ...
    if ((NULL == gpMipiAllReg) || (NULL  == gpMiscRegBase) || (NULL == gpCrgRegBase))
    {
        gpMipiAllReg = (MIPI_REGS_TYPE_S*)OSAL_IO_ADDRESS(MIPI_BASE_ADDR);   
        gpMiscRegBase = OSAL_IO_ADDRESS(MISC_BASE_ADDR);
        gpCrgRegBase = OSAL_IO_ADDRESS(CRG_BASE_ADDR);
    }
...
static inline PHY_STATUS_S mipi_drv_get_phy_status(COMBO_LINK link_id)
{
...
/* register PHY_MODE_LINK is outdated */
char * mipi_drv_get_phymode(COMBO_LINK link_id, MIPI_PHY mipi_phy)
{
    unsigned int value;
    unsigned int work_mode;
    unsigned long mipi_mode_reg = 0;
    mipi_mode_reg = (unsigned long)gpMiscRegBase + 0x0004;
    //value = osal_readl(OSAL_IO_ADDRESS(0x20120004));  // phy work mode
    value = osal_readl(mipi_mode_reg);
    work_mode = (value >> 20) & 0x3;
    if (0 == work_mode)
    {
        return ("MIPI");
    }
...
unsigned int mipi_drv_get_data(COMBO_LINK link_id)
{
	if (0 == link_id)
	{
		//return *(unsigned int*)(OSAL_IO_ADDRESS(0x20681200));
		return *(unsigned int*)((unsigned int)gpMipiAllReg + 0x1200);
	}
	else if (1 == link_id)
	{
		//return *(unsigned int*)(OSAL_IO_ADDRESS(0x20681600));
		return *(unsigned int*)((unsigned int)gpMipiAllReg + 0x1600);
	}
